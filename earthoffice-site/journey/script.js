...
async function loadData() {
  try {
    const resp = await fetch("prompts.txt?v=" + Date.now(), { cache: "no-store" });
...
async function loadResponse(prompt) {
  ...
  // Remove prompt text from header
  if (titleEl) titleEl.textContent = ""; // or: `Prompt #${prompt.PromptID ?? "—"}`;

  // Build metadata line
  const metaParts = [];
  if (prompt.PromptDate) metaParts.push(`Date: ${formatPromptDate(prompt.PromptDate)}`);
  if (Number.isFinite(prompt.HRTDays)) metaParts.push(`Day ${prompt.HRTDays}`);
  if (prompt.Class) metaParts.push(`Class: ${prompt.Class}`);
  if (prompt.Phase) metaParts.push(`Phase: ${prompt.Phase}`);
  if (prompt.SourceFileID != null) metaParts.push(`File: ${prompt.SourceFileID}`);

  if (subtitleEl) subtitleEl.textContent = metaParts.join(" · ");
  ...
  try {
    ...
    // Scroll within the response to p### token
    setTimeout(() => {
...
function updateHeaderCounters() {
  try {
    ...
    const start = new Date(2025, 8, 9, 0, 0, 1);
    const now = new Date();
...
function formatPromptDate(value) {
  // Your data looks like "2025-07-15 00:00:00"
  if (!value) return "—";

  // If it matches "YYYY-MM-DD ..." just take the date part
