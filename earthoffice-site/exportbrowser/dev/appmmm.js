...
function renderConversationList() {
  ...
  state.convList.forEach((c) => {
    ...
    left.textContent = `${fmtDate(c.firstTime)} → ${fmtDate(c.lastTime)}`;

    const right = document.createElement("span");
    right.textContent = c.turnCount === 1 ? "1 turn" : `${c.turnCount} turns`;
    ...
    // Topics (Tier 2): show assigned topics + allow editing
	const actions = document.createElement("div");
	actions.className = "convActionsRow";

	// Row 1: topics line + Topics link
	const row1 = document.createElement("div");
	row1.className = "convActionsRow1";

const topicNames = getTopicNamesForConv(c.convId);

const topicLine = document.createElement("div");
topicLine.className = "convTopicsLine";
topicLine.textContent = topicNames.length
  ? topicNames.join(", ")
  : "Unassigned";

//	const topicLine = document.createElement("div");
//	topicLine.className = "convTopicsLine";
//	topicLine.textContent = topicNames.length ? topicNames.join(", ") : "Unassigned";

	const tbtn = document.createElement("button");
	tbtn.className = "tertiary convTopicsBtn";
	...
	row1.appendChild(topicLine);
	row1.appendChild(tbtn);

	const providerLine = document.createElement("div");
	providerLine.className = "convProviderLine small";
	providerLine.textContent = providerLabel(getProviderFromConvId(c.convId));

	// Row 2: TagAssist right aligned
	const row2 = document.createElement("div");
	row2.className = "convActionsRow2";
	
	// TagAssist (conversation-level) - tags all turns in this conversation
	if (ENABLE_TAG_ASSIST) {
		const abtn = document.createElement("button");
		abtn.className = "tertiary convTagAssistBtn";
	...
	actions.appendChild(providerLine);
...
els.convSearch?.addEventListener("input", () => renderAll());
